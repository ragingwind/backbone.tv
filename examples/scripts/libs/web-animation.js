...
(function() {
  ...
  function detectFeatures() {
    ...
    style.textContent = '' + 'dummyRuleForTesting {' + 'width: calc(0px);' + 'width: -webkit-calc(0px); }';
  ...
  var Timing = function(timingDict) {
      ...
      this.iterationCount = isDefined(timingDict.iterationCount) ? timingDict.iterationCount : 1.0;
      ...
      this.playbackRate = isDefined(timingDict.playbackRate) ? timingDict.playbackRate : 1.0;
  ...
  TimingProxy.prototype = {
    extractMutableTiming: function() {
      return new Timing({
        ...
        timingFunction: this._timing.timingFunction ? this._timing.timingFunction.clone() : null,
  ...
  var configureDescriptor = function(descriptor) {
      descriptor.configurable = true, descriptor.enumerable = true;
      return descriptor;
    };

  ['startDelay', 'duration', 'iterationCount', 'iterationStart', 'playbackRate', 'direction', 'timingFunction', 'fillMode'].forEach(function(s) {
  ...
  Player.prototype = {
    ...
    set _currentTime(currentTime) {
      ...
      if (isDefined(this._pauseTime)) {
        ...
        this._timeDrift = (this.timeline.currentTime - this.startTime) * this.playbackRate - currentTime;
      }
      this._update();
      maybeRestartAnimation();
    }, get _currentTime() {
    ...
    _isPastEndOfActiveInterval: function() {
      return this.source === null || this.source._isPastEndOfActiveInterval();
    },
  ...
  TimedItem.prototype = {
    ...
    get _effectiveParentTime() {
      return
      this.parentGroup !== null && this.parentGroup.iterationTime !== null ? this.parentGroup.iterationTime : 0;
    }, get localTime() {
      ...
      return isDefined(this._duration) ? this._duration : (isDefined(this.timing.duration) ? this.timing.duration : this._intrinsicDuration());
    ...
    _updateAnimationTime: function() {
      if (this.localTime < this.timing.startDelay) {
        if (this.timing.fillMode === 'backwards' || this.timing.fillMode === 'both') {
          this.animationTime = 0;
        } else {
          this.animationTime = null;
        }
      } else if (this.localTime <= this.timing.startDelay + this.animationDuration) {
        this.animationTime = this.localTime - this.timing.startDelay;
      } else {
        if (this.timing.fillMode === 'forwards' || this.timing.fillMode === 'both') {
    ...
    _updateIterationParamsZeroDuration: function() {
      ...
      var isAtEndOfIterations = this.timing.iterationCount != 0 && this.localTime >= this.timing.startDelay;
      this.currentIteration = isAtEndOfIterations ? this._floorWithOpenClosedRange(this.timing.iterationStart + this.timing.iterationCount, 1.0) : this._floorWithClosedOpenRange(this.timing.iterationStart, 1.0);
      // Equivalent to unscaledIterationTime below.
      var unscaledFraction = isAtEndOfIterations ? this._modulusWithOpenClosedRange(this.timing.iterationStart + this.timing.iterationCount, 1.0) : this._modulusWithClosedOpenRange(this.timing.iterationStart, 1.0);
    ...
    _getAdjustedAnimationTime: function(animationTime) {
      ...
      return (this.timing.playbackRate < 0 ? (animationTime - this.animationDuration) : animationTime) * this.timing.playbackRate + startOffset;
    },
    ...
    _updateIterationParams: function() {
      var adjustedAnimationTime = this._getAdjustedAnimationTime(this.animationTime);
      var repeatedDuration = this.duration * this.timing.iterationCount;
      ...
      this.currentIteration = isAtEndOfIterations ? this._floorWithOpenClosedRange(
      adjustedAnimationTime, this.duration) : this._floorWithClosedOpenRange(
      adjustedAnimationTime, this.duration);
      var unscaledIterationTime = isAtEndOfIterations ? this._modulusWithOpenClosedRange(
      adjustedAnimationTime, this.duration) : this._modulusWithClosedOpenRange(
      adjustedAnimationTime, this.duration);
    ...
    clone: function() {
      throw new Error("Derived classes must override TimedItem.clone()");
    },
    ...
    _getLeafItemsInEffectImpl: function(items) {
      throw new Error("Derived classes must override TimedItem._getLeafItemsInEffectImpl()");
    },
    _isPastEndOfActiveInterval: function() {
      return this._inheritedTime > this.endTime;
    },
    getPlayer: function() {
      return this.parentGroup === null ? this._player : this.parentGroup.getPlayer();
    },
    _netEffectivePlaybackRate: function() {
      ...
      return this.parentGroup === null ? effectivePlaybackRate : effectivePlaybackRate * this.parentGroup._netEffectivePlaybackRate();
    },
  };
  ...
  var isCustomAnimationEffect = function(animationEffect) {
      ...
      return typeof animationEffect === "object" && animationEffect.hasOwnProperty("sample") && typeof animationEffect.sample === "function";
    };
  ...
  var Animation = function(target, animationEffect, timing, parentGroup) {
      ...
      this.name = this.animationEffect instanceof KeyframeAnimationEffect ? this.animationEffect.property : '<anon>';
    };

  Animation.prototype = createObject(TimedItem.prototype, {
    _sample: function() {
      this.animationEffect.sample(this._timeFraction, this.currentIteration, this.targetElement, this.underlyingValue);
    },
    _getLeafItemsInEffectImpl: function(items) {
      items.push(this);
    },
    clone: function() {
      return new Animation(this.targetElement, cloneAnimationEffect(this.animationEffect), this.timing.clone());
    },
    toString: function() {
      var funcDescr = this.animationEffect instanceof AnimationEffect ? this.animationEffect.toString() : 'Custom scripted function';
      return 'Animation ' + this.startTime + '-' + this.endTime + ' (' + this.localTime + ') ' + funcDescr;
    }
  });
  ...
  var TimingGroup = function(token, type, children, timing, parentGroup) {
      ...
      var childrenCopy = (children && Array.isArray(children)) ? children.slice() : [];
  ...
  TimingGroup.prototype = createObject(TimedItem.prototype, {
    ...
    clone: function() {
      ...
      return this.type === "par" ? new ParGroup(children, this.timing.clone()) : new SeqGroup(children, this.timing.clone());
    },
    ...
    toString: function() {
      return this.type + ' ' + this.startTime + '-' + this.endTime + ' (' + this.localTime + ') ' + ' [' + this.children.map(function(a) {
        return a.toString();
      }) + ']'
    },
  });
  ...
  MediaReference.prototype = createObject(TimedItem.prototype, {
    _intrinsicDuration: function() {
      ...
      return isNaN(this._media.duration) ? Infinity : this._media.duration / this._media.defaultPlaybackRate;
    },
    ...
    _updateInheritedTime: function(inheritedTime) {
      ...
      this.timing.iterationStart + this.timing.iterationCount, 1.0);
      if (this.currentIteration === finalIteration && this._timeFraction === endTimeFraction && this._intrinsicDuration() >= this.duration) {
      ...
      var playbackRate = this._media.defaultPlaybackRate * this._netEffectivePlaybackRate();
      ...
      if (this.getPlayer().paused || !this._isSeekableUnscaledTime(this.iterationTime)) {
        this._ensurePaused();
      } else {
        this._ensurePlaying();
      }
      ...
      if (Math.abs(this.iterationTime - this._unscaledMediaCurrentTime()) > delta) {
  ...
  AnimationEffect._createKeyframeFunction = function(property, value, operation) {
  ...
  PathAnimationEffect.prototype = createObject(AnimationEffect.prototype, {
    sample: function(timeFraction, currentIteration, target) {
      ...
      if (this.rotate) {
        ...
        value.push({
          t: 'rotate',
          d: [rotation / 2 / Math.PI * 360]
        });
      }
      compositor.setAnimatedValue(target, "transform", new AnimatedResult(value, this.operation, timeFraction));
    },
    clone: function() {
  ...
  /** @constructor */
  var KeyframeAnimationEffect = function(property, operation, accumulateOperation) {
      AnimationEffect.call(this, constructorToken, operation, accumulateOperation);
    ...
    sample: function(timeFraction, currentIteration, target) {
      ...
      while (i < frames.length) {
        if (frames[i].offset == timeFraction) {
          ...
          compositor.setAnimatedValue(target, this.property, new AnimatedResult(frames[i].rawValue, this.operation, timeFraction));
          return;
        }
        if (frames[i].offset > timeFraction) {
          afterFrameNum = i;
      ...
      var animationValue = interpolate(this.property, beforeFrame.rawValue, afterFrame.rawValue, localTimeFraction);
      compositor.setAnimatedValue(target, this.property, new AnimatedResult(animationValue, this.operation, timeFraction));
    },
    getValue: function(target) {
  ...
  TimingFunction.createFromString = function(spec) {
    ...
    if (bezierMatch) {
      ...
      Number(bezierMatch[1]), Number(bezierMatch[2]), Number(bezierMatch[3]), Number(bezierMatch[4])]);
    }
  ...
  var SplineTimingFunction = function(spec) {
      ...
      for (var ii = 0; ii <= 100; ii += 1) {
        var i = ii / 100;
        this.map.push([
        3 * i * (1 - i) * (1 - i) * this.params[0] + 3 * i * i * (1 - i) * this.params[2] + i * i * i, 3 * i * (1 - i) * (1 - i) * this.params[1] + 3 * i * i * (1 - i) * this.params[3] + i * i * i]);
      }
    };
  ...
  var interpArray = function(from, to, f, type) {
      console.assert(Array.isArray(from) || from === null, 'From is not an array or null');
      console.assert(Array.isArray(to) || to === null, 'To is not an array or null');
      console.assert(from === null || to === null || from.length === to.length, 'Arrays differ in length ' + from + " : " + to);
  ...
  var rectangleType = {
    ...
    toCssValue: function(value) {
      return 'rect(' + percentLengthType.toCssValue(value.top) + ',' + percentLengthType.toCssValue(value.right) + ',' + percentLengthType.toCssValue(value.bottom) + ',' + percentLengthType.toCssValue(value.left) + ')';
    },
    fromCssValue: function(value) {
  ...
  var shadowType = {
    ...
    _addSingle: function(base, delta) {
      ...
      var result = {
        ...
        base ? base.hOffset : lengthType.zero(), delta ? delta.hOffset : lengthType.zero()),
        vOffset: lengthType.add(
        base ? base.vOffset : lengthType.zero(), delta ? delta.vOffset : lengthType.zero()),
        blur: lengthType.add(
        base && base.blur || lengthType.zero(), delta && delta.blur || lengthType.zero()),
      };
      if (base && base.spread || delta && delta.spread) {
        result.spread = lengthType.add(
        base && base.spread || lengthType.zero(), delta && delta.spread || lengthType.zero());
      }
      if (base && base.color || delta && delta.color) {
        result.color = colorType.add(
        base && base.color || colorType.zero(), delta && delta.color || colorType.zero());
      }
      return result;
    },
    add: function(base, delta) {
    ...
    _interpolateSingle: function(from, to, f) {
      ...
      var result = {
        ...
        from ? from.hOffset : lengthType.zero(), to ? to.hOffset : lengthType.zero(), f),
        vOffset: lengthType.interpolate(
        from ? from.vOffset : lengthType.zero(), to ? to.vOffset : lengthType.zero(), f),
        blur: lengthType.interpolate(
        from && from.blur || lengthType.zero(), to && to.blur || lengthType.zero(), f),
      };
      if (from && from.spread || to && to.spread) {
        result.spread = lengthType.interpolate(
        from && from.spread || lengthType.zero(), to && to.spread || lengthType.zero(), f);
      }
      if (from && from.color || to && to.color) {
        result.color = colorType.interpolate(
        from && from.color || colorType.zero(), to && to.color || colorType.zero(), f);
      }
      return result;
    },
    interpolate: function(from, to, f) {
  ...
  var colorType = {
    ...
    interpolate: function(from, to, f) {
      return [interp(from[0], to[0], f), interp(from[1], to[1], f), interp(from[2], to[2], f), interp(from[3], to[3], f)];
    },
    toCssValue: function(value) {
  ...
  var extractValues = function(values, numValues, hasOptionalValue, hasUnits) {
      var result = [];
  ...
  function buildMatcher(name, numValues, hasOptionalValue, hasUnits, baseValue) {
    ...
    return [transformRE(name, numValues, hasOptionalValue), function(x) {
  ...
  var transformREs = [
  buildRotationMatcher('rotate', 1, false), buildRotationMatcher('rotateX', 1, false), buildRotationMatcher('rotateY', 1, false), buildRotationMatcher('rotateZ', 1, false), build3DRotationMatcher(), buildRotationMatcher('skew', 1, true, 0), buildRotationMatcher('skewX', 1, false), buildRotationMatcher('skewY', 1, false), buildMatcher('translateX', 1, false, true, {
    px: 0
    ...
    px: 0
  }), buildMatcher('translate3d', 3, false, true), buildMatcher('scale', 1, true, false, "copy"), buildMatcher('scaleX', 1, false, false, 1), buildMatcher('scaleY', 1, false, false, 1), buildMatcher('scaleZ', 1, false, false, 1), buildMatcher('scale3d', 3, false, false), buildMatcher('perspective', 1, false, true)];
  ...
  var transformType = {
    ...
    interpolate: function(from, to, f) {
      ...
      for (var i = 0; i < from.length; i++) {
        console.assert(from[i].t === to[i].t || from[i].t === null || to[i].t === null, 'Transform types should match or one should be the underlying value');
        ...
        case 'skewY':
          out.push({
            t: type,
            d: interp(from[i].d, to[i].d, f, type)
          });
          break;
        default:
          var result = [];
    ...
    fromCssValue: function(value) {
      ...
      while (value.length > 0) {
        ...
        if (!isDefinedAndNotNull(r)) return result;
      }
      return result;
    }
  };
  ...
  var propertyIsSVGAttrib = function(property, target) {
      return target.namespaceURI == 'http://www.w3.org/2000/svg' && property in svgProperties;
    };
        ...
        if (resultList.length > 0) {
          ...
          for (; i < resultList.length; i++) {
            ...
            case 'add':
              ...
              continue;
            case 'merge':
              baseValue = interpolate(property, baseValue, inValue, resultList[i].fraction);
              continue;
            }
          }
          var svgMode = propertyIsSVGAttrib(property, this.target);
          setValue(this.target, property, toCssValue(property, baseValue, svgMode));
          this.properties[property] = [];
        } else {
  ...
  var usePerformanceTiming = typeof performance === "object" && typeof performance.timing === "object" && typeof performance.now === "function";
  ...
  if (usePerformanceTiming) {
    var load = function() {
        ...
        documentTimeZeroAsRafTime = performance.timing.loadEventStart - performance.timing.navigationStart;
  ...
  if (document.readyState == 'complete') {
    ...
    if (!usePerformanceTiming) {
      console.warn('Web animations can\'t discover document zero time when ' + 'asynchronously loaded in the absence of performance timing.');
    }
    load();
  } else {
  ...
  var documentTime = function() {
      ...
      if (!isDefined(cachedDocumentTimeMillis)) {
        cachedDocumentTimeMillis = relativeTime(clockMillis(), documentTimeZeroAsClockTime);
        setTimeout(function() {
          cachedDocumentTimeMillis = undefined;
        }, 0);
      }
      return cachedDocumentTimeMillis === null ? null : cachedDocumentTimeMillis / 1000;
    };
